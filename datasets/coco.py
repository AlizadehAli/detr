...
kitti_obj_map = \
    {'Car': 0, 'Van': 1, 'Truck': 2, 'Pedestrian': 3, 'Person_sitting': 4, 'Cyclist': 5, 'Tram': 6}
        ...
        with open(ann_file) as file:
            for line_num,line in enumerate(file):
                ...
                if obj_class == "DontCare" or obj_class == "Misc":
                    continue
                classes.append(kitti_obj_map[obj_class])
...
def convert_to_cxcywh(xmin, ymin, xmax, ymax, img_w, img_h):
    # dw = 1./(img_w)
    # dh = 1./(img_h)
    x = (xmin + xmax)/2.0 - 1
    y = (ymin + ymax)/2.0 - 1
