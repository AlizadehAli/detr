...
model_path = './checkpoint/coco/checkpoint0099.pth'
...
for i in range(len(bboxes_scaled)):
    u_img = bboxes_scaled[i][0].cpu().detach().numpy()#(bboxes_scaled[i][0].cpu().detach().numpy() + bboxes_scaled[i][2].cpu().detach().numpy())/2
    v_img = bboxes_scaled[i][1].cpu().detach().numpy()#(bboxes_scaled[i][1].cpu().detach().numpy() + bboxes_scaled[i][3].cpu().detach().numpy())/2
    idxs.append((int(u_img), int(v_img)))

# idxs = [(600, 200), (580, 120)]
...
for idx_o, ax in zip(idxs, axs):
    ...
    ax.imshow(sattn[..., idx[1], idx[0]], cmap='cividis', interpolation='nearest')
    ax.axis('off')
